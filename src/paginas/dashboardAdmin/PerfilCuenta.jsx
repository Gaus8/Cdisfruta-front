...
export default function PerfilCuenta({ adminMode = true }) {
  ...
  return <div className={tw(!adminMode ? 'min-h-screen bg-slate-50' : '')}>
    ...
    {!adminMode && <button type="button" onClick={() => navigate('/cliente/tienda')} className={tw('inline-flex min-h-10 items-center gap-2 text-sm font-semibold text-slate-600 hover:text-[#e06d43]')}><FaArrowLeft /> Volver a la tienda</button>}
