...
export default function Inventario() {
  ...
  const [form, setForm] = useState({ nombre: '', categoria: 'Materia prima', stock: '', codigoBarras: '', foto: null });
  ...
  const saveProduct = async (event) => {
    ...
    try {
      ...
      setMessage({ type: 'success', text: `${data.product.nombre} se agregó al inventario.` });
      setForm({ nombre: '', categoria: 'Materia prima', stock: '', codigoBarras: '', foto: null });
      setPreview('');
  ...
  const exportSelectedToCatalog = async () => {
    if (!selectedIds.length) return;
    try {
      const { data } = await apiAxios.patch('/admin/inventario/exportar-catalogo', { ids: selectedIds });
      const exported = new Map(data.products.map((product) => [product._id, product]));
      setProducts((items) => items.map((item) => exported.get(item._id) || item));
      setSelectedIds([]);
      setMessage({ type: 'success', text: `${data.products.length} artículo(s) exportados al catálogo como borrador. Completa sus datos comerciales desde Catálogo.` });
    } catch (error) { setMessage({ type: 'error', text: toError(error) }); }
  };
  ...
  const registerScannedItem = () => {
    setScannerOpen(false);
    setForm({ nombre: '', categoria: 'Materia prima', stock: '', codigoBarras: scannerCode, foto: null });
    setPreview('');
    if (fileRef.current) fileRef.current.value = '';
    setScannerCode('');
    setScannedProduct(null);
  ...
  const removeItem = async () => {
    ...
    try {
      ...
      setSelectedIds((items) => items.filter((id) => id !== deleteTarget._id));
  ...
  return <section className={tw('mx-auto w-full max-w-7xl space-y-6 pb-12')}>
    ...
    <div className={tw('overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm')}>
      <div className={tw('flex flex-col gap-3 border-b border-slate-100 p-4 sm:flex-row sm:items-center sm:justify-between sm:px-6')}><div><h2 className={tw('text-lg font-bold text-slate-800')}>Existencias</h2><p className={tw('text-sm text-slate-500')}>Selecciona artículos terminados para prepararlos en el catálogo.</p></div><label className={tw('relative block w-full sm:max-w-xs')}><FaSearch className={tw('absolute left-3 top-1/2 -translate-y-1/2 text-slate-400')} /><input className={tw(inputClass, 'pl-9')} value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar producto o código" /></label></div>
      {selectedIds.length > 0 && <div className={tw('flex flex-col gap-3 border-b border-orange-100 bg-orange-50/70 px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-6')}><p className={tw('text-sm font-medium text-slate-700')}>{selectedIds.length} artículo(s) seleccionado(s) para exportar</p><button type="button" onClick={exportSelectedToCatalog} className={tw(buttonClass, 'bg-[#ff7e5f] text-white hover:bg-[#e06d43]')}><FaBoxes /> Exportar al catálogo</button></div>}
      {filtered.length === 0 ? <div className={tw('px-6 py-14 text-center')}><FaBoxes className={tw('mx-auto mb-3 text-3xl text-slate-300')} /><p className={tw('font-semibold text-slate-700')}>{products.length ? 'No hay coincidencias' : 'Aún no hay productos en inventario'}</p><p className={tw('mt-1 text-sm text-slate-500')}>{products.length ? 'Prueba otro nombre o código de barras.' : 'Registra el primer producto para comenzar.'}</p></div> : <>
        <div className={tw('hidden overflow-x-auto md:block')}>
          <table className={tw('w-full min-w-[900px] text-left')}>
            <thead className={tw('bg-slate-50 text-xs uppercase tracking-wide text-slate-500')}><tr>
              <th className={tw('px-4 py-3')}>Catálogo</th><th className={tw('px-4 py-3')}>Artículo</th><th className={tw('px-4 py-3')}>Código de barras</th><th className={tw('px-4 py-3')}>Creación</th><th className={tw('px-4 py-3')}>Existencias</th><th className={tw('px-5 py-3 text-right')}>Ajustar</th>
            </tr></thead>
            <tbody className={tw('divide-y divide-slate-100')}>
              {filtered.map((product) => <tr key={product._id} className={tw('hover:bg-slate-50')}>
                <td className={tw('px-4 py-3')}><input type="checkbox" aria-label={`Seleccionar ${product.nombre} para catálogo`} checked={selectedIds.includes(product._id)} disabled={product.enCatalogo !== false} onChange={() => toggleSelected(product._id)} className={tw('h-4 w-4 accent-[#ff7e5f] disabled:cursor-not-allowed disabled:opacity-30')} /></td>
                <td className={tw('px-4 py-3')}><div className={tw('flex items-center gap-3')}><img src={product.imagen} alt="" className={tw('h-12 w-12 rounded-xl bg-slate-100 object-cover')} /><div><p className={tw('font-semibold text-slate-800')}>{product.nombre}</p><p className={tw('text-xs text-slate-500')}>{product.categoria}</p><span className={tw('mt-1 inline-flex rounded-full px-2 py-0.5 text-[10px] font-semibold', product.enCatalogo === false ? 'bg-slate-100 text-slate-600' : product.publicarEnTienda === false ? 'bg-amber-100 text-amber-800' : 'bg-orange-100 text-orange-800')}>{product.enCatalogo === false ? 'Solo inventario' : product.publicarEnTienda === false ? 'Borrador en catálogo' : 'Tienda virtual'}</span></div></div></td>
                <td className={tw('px-4 py-3')}><div className={tw('flex gap-1')}><input aria-label={`Código de barras de ${product.nombre}`} maxLength={64} className={tw(inputClass, 'min-w-36 px-2 py-2 text-xs')} placeholder="Asignar código" value={barcodeValues[product._id] ?? product.codigoBarras ?? ''} onChange={(e) => setBarcodeValues((old) => ({ ...old, [product._id]: e.target.value }))} /><button type="button" onClick={() => saveBarcode(product)} className={tw('rounded-lg bg-slate-100 px-2 text-xs font-semibold text-slate-700 hover:bg-emerald-100')}>Guardar</button></div></td>
                <td className={tw('px-4 py-3 text-sm text-slate-600')}>{new Date(product.fechaCreacion || product.createdAt).toLocaleDateString('es-CO')}</td>
                <td className={tw('px-4 py-3')}><span className={tw('rounded-full px-3 py-1 text-sm font-bold', product.stock <= 5 ? 'bg-amber-100 text-amber-800' : 'bg-emerald-100 text-emerald-800')}>{product.stock} uds.</span></td>
                <td className={tw('px-5 py-3')}><div className={tw('flex justify-end gap-2')}><button type="button" disabled={updatingId === product._id || product.stock <= 0} onClick={() => setStock(product, -1)} aria-label={`Restar una unidad a ${product.nombre}`} className={tw('rounded-lg border border-slate-200 p-2 text-slate-600 hover:bg-slate-100 disabled:opacity-40')}><FaMinus /></button><button type="button" disabled={updatingId === product._id} onClick={() => setStock(product, 1)} aria-label={`Sumar una unidad a ${product.nombre}`} className={tw('rounded-lg bg-[#ff7e5f] p-2 text-white hover:bg-[#e06d43] disabled:opacity-40')}><FaPlus /></button><button type="button" onClick={() => { setManualTarget(product); setManualQuantity(1); setManualDirection('ingreso'); }} aria-label={`Ajustar cantidad de ${product.nombre}`} className={tw('rounded-lg border border-slate-200 p-2 text-slate-600 hover:bg-slate-100')}><FaEdit /></button><button type="button" onClick={() => setDeleteTarget(product)} aria-label={`Retirar ${product.nombre}`} className={tw('rounded-lg border border-rose-200 p-2 text-rose-600 hover:bg-rose-50')}><FaTrash /></button></div></td>
              </tr>)}
            </tbody>
          </table>
        </div>
        <div className={tw('divide-y divide-slate-100 md:hidden')}>
          {filtered.map((product) => <article key={product._id} className={tw('flex gap-3 p-4')}>
            <input type="checkbox" aria-label={`Seleccionar ${product.nombre} para catálogo`} checked={selectedIds.includes(product._id)} disabled={product.enCatalogo !== false} onChange={() => toggleSelected(product._id)} className={tw('mt-1 h-4 w-4 shrink-0 accent-[#ff7e5f] disabled:opacity-30')} />
            <img src={product.imagen} alt="" className={tw('h-16 w-16 shrink-0 rounded-xl bg-slate-100 object-cover')} />
            <div className={tw('min-w-0 flex-1')}><div className={tw('flex items-start justify-between gap-2')}><div className={tw('min-w-0')}><h3 className={tw('truncate font-semibold text-slate-800')}>{product.nombre}</h3><p className={tw('text-xs text-slate-500')}>{product.categoria}</p><span className={tw('text-[10px] font-semibold', product.enCatalogo === false ? 'text-slate-500' : product.publicarEnTienda === false ? 'text-amber-700' : 'text-orange-700')}>{product.enCatalogo === false ? 'Solo inventario' : product.publicarEnTienda === false ? 'Borrador en catálogo' : 'Tienda virtual'}</span></div><span className={tw('shrink-0 rounded-full px-2.5 py-1 text-xs font-bold', product.stock <= 5 ? 'bg-amber-100 text-amber-800' : 'bg-emerald-100 text-emerald-800')}>{product.stock} uds.</span></div>
              <div className={tw('mt-2 flex gap-1')}><input aria-label={`Código de barras de ${product.nombre}`} maxLength={64} className={tw(inputClass, 'min-w-0 px-2 py-2 text-xs')} placeholder="Asignar código" value={barcodeValues[product._id] ?? product.codigoBarras ?? ''} onChange={(e) => setBarcodeValues((old) => ({ ...old, [product._id]: e.target.value }))} /><button type="button" onClick={() => saveBarcode(product)} className={tw('shrink-0 rounded-lg bg-slate-100 px-2 text-xs font-semibold text-slate-700')}>Guardar</button></div>
              <div className={tw('mt-3 flex items-center justify-between')}><span className={tw('text-xs text-slate-400')}>{new Date(product.fechaCreacion || product.createdAt).toLocaleDateString('es-CO')}</span><div className={tw('flex gap-2')}><button type="button" disabled={updatingId === product._id || product.stock <= 0} onClick={() => setStock(product, -1)} className={tw('h-10 w-10 rounded-lg border border-slate-200 text-slate-600 disabled:opacity-40')} aria-label="Restar unidad"><FaMinus className={tw('mx-auto')} /></button><button type="button" disabled={updatingId === product._id} onClick={() => setStock(product, 1)} className={tw('h-10 w-10 rounded-lg bg-[#ff7e5f] text-white disabled:opacity-40')} aria-label="Sumar unidad"><FaPlus className={tw('mx-auto')} /></button><button type="button" onClick={() => { setManualTarget(product); setManualQuantity(1); setManualDirection('ingreso'); }} aria-label={`Ajustar cantidad de ${product.nombre}`} className={tw('h-10 w-10 rounded-lg border border-slate-200 text-slate-600')}><FaEdit className={tw('mx-auto')} /></button><button type="button" onClick={() => setDeleteTarget(product)} aria-label={`Retirar ${product.nombre}`} className={tw('h-10 w-10 rounded-lg border border-rose-200 text-rose-600')}><FaTrash className={tw('mx-auto')} /></button></div></div>
            </div>
          </article>)}
        </div>      </>}
    </div>

    {modalOpen && createPortal(<div className={tw('fixed inset-0 z-[2000] flex items-end justify-center bg-slate-950/55 p-0 backdrop-blur-sm sm:items-center sm:p-4')} onMouseDown={(event) => { if (event.target === event.currentTarget) setModalOpen(false); }}><form onSubmit={saveProduct} className={tw('max-h-[calc(100vh-1rem)] w-full max-w-2xl space-y-4 overflow-y-auto rounded-t-3xl bg-white p-5 shadow-2xl sm:max-h-[calc(100vh-2rem)] sm:rounded-3xl sm:p-7')}><div className={tw('flex items-start justify-between')}><div><p className={tw('text-sm font-semibold text-[#e06d43]')}>Nuevo registro</p><h2 className={tw('mt-1 text-2xl font-bold text-slate-800')}>Agregar al inventario</h2><p className={tw('mt-1 text-sm text-slate-500')}>Registra cualquier artículo y sus existencias.</p></div><button type="button" onClick={() => setModalOpen(false)} className={tw('rounded-full p-2 text-slate-500 hover:bg-slate-100')} aria-label="Cerrar"><FaTimes /></button></div>
      <div className={tw('grid gap-4 sm:grid-cols-2')}>
        ...
        <label className={tw('space-y-1.5 text-sm font-medium text-slate-700')}>Categoría<select className={tw(inputClass)} value={form.categoria} onChange={(e) => setForm({ ...form, categoria: e.target.value })}><option>Materia prima</option><option>Empaque</option><option>Insumo de producción</option><option>Producto terminado</option><option>Otro insumo</option></select></label>
